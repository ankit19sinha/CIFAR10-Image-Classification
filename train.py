...
parser.add_argument('--finetuning', type=bool, help='set requires_grad=False for non-updatable pretrained resnet18 model')
...
if args.model == 'softmax':
    # (resnet18 + no hidden layer, input -> output)
    model = models.softmax.Softmax(im_size, n_classes, args.finetuning)
elif args.model == 'twolayernn':
    # (resnet18 + one hidden layer, input -> hidden layer -> output)
    model = models.twolayernn.TwoLayerNN(im_size, args.hidden_dim, n_classes, args.finetuning)
elif args.model == 'convnet':
    # (resnet18 + 1 convolution layer, input -> hidden layers -> output)
    model = models.convnet.CNN(im_size, args.hidden_dim, args.kernel_size, n_classes, args.finetuning)
elif args.model == 'mymodel':
    # (resnet18 + 4 convolution layers, input -> hidden layers -> output)
    model = models.mymodel.MyModel(im_size, args.hidden_dim, args.kernel_size, n_classes, args.finetuning)
else:
...
def train(epoch):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if args.cuda: images, targets = images.cuda(), targets.cuda()
