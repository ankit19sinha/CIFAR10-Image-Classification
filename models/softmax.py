...
class Softmax(nn.Module):
	def __init__(self, im_size, n_classes, finetuning):
		super(Softmax, self).__init__()
		# Retrieve the pretrained ResNet18 model
		self.softmax_resnet = resNet.resnet18(pretrained = True)

		# Set requires_grad = False if we don't want to update the weights
		if(not finetuning):
			for param in self.softmax_resnet.parameters():
				param.requires_grad = False

		# Override the original fully-connected layer of ResNet18 with my own
		self.softmax_resnet.fc = nn.Linear(512, n_classes)
	...
	def forward(self, images):
		...
		x = self.softmax_resnet(images.view(images.shape[0], 3, 224, 224))
